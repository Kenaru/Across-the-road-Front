...
const FeedbackItem = ({ content, name, title, img }) => (
    <Box
        ...
        margin="1rem"
        w="300px"
        maxW="90%" // Limit the width to 90% of the container
        h="auto"
    ...
    >
        <Flex direction="column" justify="space-between" p="1rem">
            <Image src={img} alt={name} w="80px" h="80px" borderRadius="full" objectFit="cover" />
            <Text fontSize="16px" color="azure" mb="1rem">{content}</Text>
            <Text color="azure">{name} - <span>{title}</span></Text>
        </Flex>
    </Box>
);
